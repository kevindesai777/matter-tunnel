...
**Install `nrfutil` from Nordic's own installer, never from pip or Homebrew** —
those give you the legacy Python tool, which has no `sdk-manager`. Then
...
`west zephyr-export` **adds** a second CMake package registration rather than
clobbering an existing one — CMake keys by path hash, and `west build` resolves
`ZEPHYR_BASE` from the workspace topdir — so a `/opt` install and this workspace
coexist.
...
Expect **797,604 B flash (81.83%)** and **167,452 B RAM (63.88%)** for the full
signed build. If the flash figure lands near 79.8% you have a *sleepy* device and
the wrong device under test — check `CONFIG_OPENTHREAD_MTD_SED` and
...
`client_key.pem` is not in the repository, and must not be — it is a private key
...
# BLE floor, run from the Pi (see gotcha 6 — this cannot be done from macOS)
...
`InvokeCommandRequest` to RX of `InvokeCommandResponse`, so harness overhead is
excluded. Conditions are interleaved rather than blocked, because the effect being
measured (~40 ms) is smaller than the observed spread, and blocking would let RF
drift correlate with the treatment.

### On-device benchmark

The primary cost evidence, and the lowest-noise instrument: `tunnel_bench.cpp`
measures sign and verify with the Cortex-M DWT cycle counter, sidestepping the
network entirely. Build with `bench.conf` / `bench-cc310.conf` from
...
5. **AP-Protect may be enabled on a new DK.** `west flash` fails with *"The
   ...
   precious is lost — this build uses test attestation, not a production DAC.
...
7. **BLE from Python on macOS is impossible without an app bundle.** `bleak` aborts
   ...
   binary's `Info.plist`, so CoreBluetooth never even prompts. Do not retry it — run
   the BLE harness from the Pi.
...
9. **chip-tool log parsing:** strip the whole ANSI CSI family
   (`\x1b\[[0-9;]*[A-Za-z]`), not just colour codes ending in `m` — `\x1b[0J`
...
11. **`nrfutil device program` will not take a `.hex` for the dongle.** For a
    ...
    — check `nrfutil device list` before concluding a flash failed, because a
    successful flash leaves DFU mode and the *next* call is the one that errors.
...
15. **A USB extender can kill the RCP silently, and `systemctl` will still report
    ...
    events at all* — no over-current, no reset attempt. `otbr-agent` is a sysv unit
...
19. **Raspberry Pi OS Trixie is PEP-668 externally managed** — install pyserial with
    ...
    terminal is required to read the password"* — use `ssh -t`. And `lsb_release -i`
