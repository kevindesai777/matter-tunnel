...
byte tunnel**, plus an application-layer signing layer that authenticates payloads
end to end. An intermediating Matter hub relays bytes it can neither interpret nor
forge — so the hub keeps its role as a router and loses its role as an authority.

Built clean-room against the public Matter specification and SDK. The component
carries no device-type semantics and is intended to drop into any Matter
application; the door lock here is one example of it, not the point of it.
...
- **Network isolation does not revoke authority.** With the device moved entirely
  ...
  its traffic relayed by *our* border router, which advertised the hub's fabric
  record onto the LAN. Matter authority is an IP-layer relationship; Thread
  membership is link-layer. They are independent, so topology cannot revoke
  authority. Only fabric removal or a factory reset can.
...
intermediaries is older still — JEDI (USENIX Security 2019), OSCORE (RFC 8613).
...
drift cannot correlate with the treatment, **zero errors**, on nRF52840. Every
number below is regenerated from the raw data by `measure/analyse.py` — nothing in
this table is transcribed by hand.
...
**Signing cost is invariant to the transport carrying it.** The two Matter
confidence intervals overlap, and the border-router interval *contains* the isolated
on-device benchmark of 39.9 ms measured with the Cortex-M DWT cycle counter. Three
instruments, the strongest two agreeing to within the width of a confidence interval.

⚠️ **Do not quote the BLE delta as the cost of signing.** It is quantised by the
15 ms connection interval: 977/1000 unsigned samples land in the 30 ms bin (two
connection events) and 945/1000 signed samples in the 60 ms bin (four). The
link-layer schedule sets the latency, not the processing, so a ~40 ms operation
appears as exactly two extra events. `measure/analyse.py --interval 15` prints this.
...
A commercial hub costs about 13 ms — roughly an eighth of the path. Matter over
Thread with a border router you control costs about 61 ms, some 4.7× more. Removing
the hub from the device's trust boundary is affordable **because the hub was never
the expensive part**. Without the control condition the 103 ms would have been
reported as hub latency, wrong by a factor of five.

⚠️ The 13 ms is an **upper bound** on hub overhead, not a clean measurement of it:
the commercial hub's link margin was never characterised at comparable distance, and
this bench showed link margin alone moving p50 by 6 ms and p99 by 64 ms.
...
Wire overhead is 7 → 79 bytes. The identity bundle is 1060 B, about 90% of it X.509
— key distribution here is a *certificate-transport* problem rather than a key
problem — fetched once at client setup in four chunks, never per command.
...
Three results worth stating plainly. **Ed25519 in software beats CC310-accelerated
P-256** on both operations. **Enabling CC310 makes Ed25519 unavailable** — the CC3XX
driver claims ECC key operations and does not implement Twisted Edwards, so PSA
returns `PSA_ERROR_NOT_SUPPORTED`. And CC310 costs about 36 KB of flash to get there.
Ed25519 is also far tighter in distribution (16.67–16.73 ms against P-256's
16.8–55.7 ms from rejection-sampled nonces), which matters when the figure being
reported is p99.
...
lib/tunnel/         the component — transport-agnostic core, signing, replay,
                    identity, client-key registration, and two transport adapters
...
Extensions). Note §7.21, not §7.19 — §7.19 was correct only through Matter 1.3.
...
The cluster is driven by a `chip-tool` built from the **unmodified** Matter SDK,
invoking by raw identifier — so any specification-conformant client can drive it,
not only one built from this definition:
...
length-prefixed to remove concatenation ambiguity. Each field was verified bound by
tampering with it — altered counter, flipped direction, tampered payload,
substituted device key — and each is rejected.

**Anti-replay: a monotonic counter with a reserved ceiling.** Challenge-response
would cost an extra round trip — about 250 ms measured, some six times the entire
signing cost — and would inflate the very latency being attributed to signing. But
the naive counter is unsafe: persisting "highest seen" makes the stored value
*regress* after a reboot that follows unpersisted acceptances, reopening the replay
window. Instead the device persists a reserved ceiling (`ceiling + 64`) **before**
accepting and keeps the floor in RAM. The floor never moves backwards across a
reboot, and flash is written once per 64 commands. A reset can only lose an
acceptance, which is safe; never a reservation, which would not be. Resync needs no
extra exchange — a rejection returns the device's floor.
...
hub-relayed tunnel itself. No out-of-band channel, no vendor backend, no admin
rights: this matters because the intended audience is small makers.
...
*silently* on a re-pair — a nasty failure mode, because the device keeps working
until someone re-adds it.
...
The **Matter SDK commit, not the SDK tag**, is the meaningful reproducibility pin,
and it is identical in the `/opt` v3.4.0 tree and in the add-on workspace.

### One lesson worth repeating

An earlier version of this work measured a build whose sleepy-end-device setting came
from a config overlay applied to a build directory and recorded in **no tracked
file**. A fresh build from the repository produced a *sleepy* device with a ~2.3 s
polling floor — not the firmware the measurements were taken on. Both options are now
written explicitly into `examples/lock/prj.conf`, and a clean build from this
repository has been checked to reproduce the measured configuration and footprint.

**A build directory is not a record.** If a measured artifact's configuration is not
in a tracked file, the measurement is not reproducible, and deleting a build
directory silently changes the device under test.
