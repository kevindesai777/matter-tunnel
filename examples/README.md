...
3. **Copy `prj.conf`**, changing the product name, the product ID and the ZAP path.
   ...
   intend to compare latency against the numbers in the top-level README. A sleepy end
   device has a ~2.3 s polling floor and is a different device under test.
...
Flash is the binding constraint on nRF52840, where the lock example lands at 81.83%
with the full security layer, so a port onto a larger stock application may need
